...
class UpdateProductRecommendationView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        select
            ...
            case
                when a.source = 'D' then (e.rating + coalesce(h.sum,0)) / (coalesce(h.count,0) + 1)
                when coalesce(
                    a.source,
                    'P'
                )= 'P' then coalesce(
        ...
        from
            ...
            a.product_id = g.product_id left join view_product_rating_sum h on a.product_id = h.product_id
        where
            a.product_type = '0'
