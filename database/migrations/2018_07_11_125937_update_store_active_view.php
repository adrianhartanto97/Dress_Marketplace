...
class UpdateStoreActiveView extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        select
            ...
            coalesce(f.sold,0) as sold_product,
            coalesce(g.transaction,0) as transaction
        from
            store a inner join store_supporting_documents b on
            a.store_id = b.store_id inner join master_province c on
            a.province = c.province_id inner join master_city d on
            a.city = d.city_id left join view_store_rating e on 
            a.store_id = e.store_id left join view_store_sold_product f on 
            a.store_id = f.store_id left join view_store_transaction g on
            a.store_id = g.store_id
        where
            a.store_active_status = '1'
        ");
    }
