...
class UpdateProductView7 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        select
            ...
            case
                when a.source = 'D' then (o.rating + coalesce(q.sum,0)) / (coalesce(q.count,0) + 1)
                when coalesce(
                    a.source,
                    'P'
                )= 'P' then coalesce(
            ...
            case
                ...
                when coalesce(a.source,'P') = 'P' then n.max_price
            end as max_price
        from
            ...
            a.product_id = o.dress_id left join product_price_attribute p on 
            o.price = p.price_id left join view_product_rating_sum q on a.product_id = q.product_id
            ;
            ");
    }
    ...
    public function down()
    {
        ...
        from
            ...
            a.product_id = m.product_id left join view_product_first_price n on
            a.product_id = n.product_id left join data_training o on
            a.product_id = o.dress_id left join product_price_attribute p on o.price = p.price_id;
            ");
    }
}
